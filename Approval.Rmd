...
##  This second graph uses the same data, but now weighted for recency, sample size and methodology.
...
yes_data <- filter(president_long, response == "yes")
...
# NO model
no_data <- filter(president_long, response == "no")
...
ggplot(loess_all, aes(x = end_date, y = percentage, color = response)) +
  ...
  labs(title = "Polling Trends Weighted for Sample Size with 95% Confidence Bands",
       x = "Poll End Date",
       y = "Response (%)",
       color = "Response",
       fill = "Response") +
...
ggplot(president_long, aes(x = end_date, y = percentage, color = response)) +
  ...
  geom_text(data = label_points,
            ...
            hjust = -0.1, vjust = -0.2, size = 3, show.legend = FALSE) +
  scale_color_manual(values = c("yes" = "red", "no" = "black")) +
  labs(title = "Probability Panel Polls with 95% Confidence Bands and Latest Labels",
       x = "Poll End Date",
       y = "Response (%)",
       color = "Response") +
  theme_minimal() +
...
```{r}
ggplot(president, aes(x = end_date, y = weight)) +
